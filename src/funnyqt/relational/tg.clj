...
(defn valueo
  ...
  overridden when transforming into the direction of the of `m` in a
  bidirectional transformation.  (You normally shouldn't use that parameter
  directly, but use the API generated by `generate-schema-relations`.)"
  ([g ae at val]
...
(defn ^:private create-reference-relation
  ...
  [role _ prefix]
  (let [role-rel-sym (symbol (str prefix "->" (clojure.string/replace (name role) "_" "-")))]
    `(defn ~role-rel-sym
